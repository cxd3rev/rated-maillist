...
def main():
    ...
    deezer = json.loads((ROOT / "data" / "deezer_albums.json").read_text(encoding="utf-8"))
    by_norm = {}
    ...
    ids = json.loads((ROOT / "data" / "deezer_ids.json").read_text(encoding="utf-8"))
    extras = {
    ...
    js = "const albums = [\n\n" + body + "\n\n];\n"
    (ROOT / "data" / "albums_generated.js").write_text(js, encoding="utf-8")
    source = (ROOT / "js" / "script.js").read_text(encoding="utf-8")
    head, rest = source.split("const albums = [", 1)
    _, tail = rest.split("];", 1)
    (ROOT / "js" / "script.js").write_text(head + js + tail, encoding="utf-8")
