...
// Flip this to true on launch day so waitlist seats can log in with a gold star.
const RATED_IS_PUBLISHED = false;
...
async function syncLaunchFlag() {

    try {

        const response =
            await apiFetch(`${API_URL}/waitlist/count`);
...
function getAlbumFriendsRating(albumId) {
    ...
    return averageScores(
        getFollowIds().map(userId =>
            getUserAlbumRating(userId, albumId)
        )
    );
...
function isFounderUser(user) {
    ...
    return Boolean(
        user &&
        (user.is_founder === true || user.is_founder === 1)
    );

}


function appendGoldStar(parent, user) {
    ...
    star.title = "Believed in RATED before launch";
    star.setAttribute("aria-label", "Early believer");
    star.textContent = "★";
...
function goldStarMarkup(user) {
    ...
    return `<span class="gold-star" title="Believed in RATED before launch" aria-label="Early believer">★</span>`;
...
function appIsLive() {
    ...
    return (
        RATED_IS_PUBLISHED ||
        localStorage.getItem("ratedLaunched") === "1"
    );
...
function updateProfileButton() {
    ...
    button.textContent = "";
    button.appendChild(
        document.createTextNode(currentUser.username)
    );
    appendGoldStar(button, currentUser);
        ...
        >
            ...
            <div class="score-column">
                ...
                <span
                    ...
                    ${
                        friendsRating !== null
                        ? `style="${scoreColorStyle(friendsRating)}"`
                        : ""
                    }
                >
...
function openAlbum(albumId, fromArtist = false, fromFriend = false) {
                ...
                <div class="album-stat-row">
                    ...
                    <div class="album-stat">
                        ...
                        <strong
                            ${
                                friendsScore !== null
                                ? `style="${scoreColorStyle(friendsScore)}"`
                                : ""
                            }
                        >
...
function getAlbumAverageFromMap(albumId, ratingsMap) {
...
function getFollowStorageKey() {
    ...
    return "ratedFollows_" + currentUser.id;
...
function toggleFollow(userId, event) {
...
function getUserRatingsMap(userId) {
...
function getUserAlbumRating(userId, albumId) {
...
function friendAvatarMarkup(user, extraClass) {
    ...
    if (user.profile_picture) {
        ...
        return `
            <div class="friend-avatar ${extraClass || ""}">
...
function followButtonMarkup(userId) {
    ...
    return `
        <button
            type="button"
            class="follow-button ${following ? "is-following" : ""}"
            onclick="toggleFollow(${userId}, event)"
        >
...
function createFriendRow(user, extra) {
    ...
    return `
        ...
        >
            ...
            <div>
                ...
                <strong>
                    ${escapeHtml(user.username)}${goldStarMarkup(user)}
                </strong>

                <p>
                    ${ratedCount}
                    album${ratedCount === 1 ? "" : "s"} rated
                    ${extra || ""}
                </p>
        ...
        list.innerHTML = `
            <div class="friend">
                ...
                <div>
                    ...
                    <p>Search a username above and follow them.</p>
                </div>
            </div>
        `;
        ...
        results.innerHTML = `
            <div class="friend">
                ...
                <div>
                    ...
                    <p>Try another username. They need an account on this device.</p>
                </div>
            </div>
        `;
...
function getUserTopAlbums(userId, limit) {

    return albums
        .map(album => ({
            album,
            score: getUserAlbumRating(userId, album.id)
        }))
        .filter(entry => entry.score !== null)
        .sort(
            (a, b) =>
                b.score - a.score ||
                b.album.year - a.album.year
        )
        .slice(0, limit);
...
function getUserTopArtists(userId, limit) {

    return getArtistDirectory()
        .map(artist => ({
            artist,
            score: averageScores(
...
function createFriendAlbumCard(album, score, rank) {

    return `

        <div
            class="album-card"
            onclick="openAlbum(${album.id}, false, true)"
        >
...
function createFriendArtistCard(artist, score, rank) {
...
function openFriendProfile(userId) {
    ...
    albumsBox.innerHTML =
        ...
        ? topAlbums
            ...
            .join("")
        : `<div class="empty-state">They haven't rated any albums yet.</div>`;


    const topArtists =
        getUserTopArtists(user.id, 12);
    ...
    artistsBox.innerHTML =
        ...
        ? topArtists
            ...
            .join("")
        : `<div class="empty-state">No artist averages yet.</div>`;
...
async function promoteLocalWaitlistUser(email, password) {

    const pending = getWaitlistUsers();
    const match = pending.find(item => item.email === email);
    ...
    const passwordHash =
        await hashSecret(match.salt + password);

    if (passwordHash !== match.password_hash) {
    ...
    if (!appIsLive()) {
        ...
        throw new Error(
            "RATED isn't live yet. We'll email you the second it drops."
        );

    }

    const locals = getLocalUsers();
    const already = locals.find(
        item =>
            item.email === email ||
...
async function createLocalAccount(username, email, password, profilePicture) {
    ...
    const reserved = getWaitlistUsers().some(
        item =>
            item.email === email ||
            item.username.toLowerCase() ===
                username.toLowerCase()
    ...
    if (reserved && !appIsLive()) {

        throw new Error(
            "That name is reserved for a pre-launch believer. Log in after RATED drops."
        );
...
async function loginLocalAccount(email, password) {
    ...
    if (!user) {
        ...
        user = await promoteLocalWaitlistUser(email, password);
...
function showAccount() {
    ...
    appendGoldStar(title, currentUser);
    ...
    document
        ...
        isFounderUser(currentUser)
            ? `${currentUser.email} · early believer`
            : currentUser.email;
